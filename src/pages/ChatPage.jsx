import { FaBars, FaChevronUp, FaPlus, FaSearch } from "react-icons/fa"
    ...
    return <div className="flex">
        ...
        <div className={`${currentChat && "hidden md:flex"} flex w-10 h-10 rounded-full justify-center items-center fixed bottom-1 left-1 bg-black/30 cursor-pointer`}>
            ...
            <div className="absolute flex bottom-13 left-0 flex-col">
                ...
                <div className={`w-10 absolute duration-200 ${toggleMenu ? "bottom-12" : "bottom-0 opacity-0 pointer-events-none"} h-10 flex justify-center items-center rounded-full overflow-hidden bg-black/30`}><FaGear size={22} /></div>
                <div className={`w-10 absolute duration-200 ${toggleMenu ? "bottom-0" : "bottom-0 opacity-0 pointer-events-none"} h-10 rounded-full overflow-hidden bg-black/30`}><img src={picture} alt={name} /></div>
            </div>
        </div>
        <div className={`h-screen  w-full md:w-[600px] shadow shadow-black/50 overflow-y-scroll scroll-none ${currentChat && "hidden md:block"}`}>
            <div className="h-[60px] sticky z-[1] top-0 bg-[#222] flex items-center p-2">
